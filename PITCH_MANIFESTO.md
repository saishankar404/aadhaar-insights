...
**Team**: [Team Name]
**Date**: January 2026
**Challenge**: UIDAI Aadhaar Data Analytics Challenge
...
```
CSV Files (UIDAI Data)
    ↓
DataLoader (Load & Concatenate)
    ↓
MetricEngine (Calculate ASR, UII, TDS, CBCG, AEPG)
    ↓
AnomalyDetector (Isolation Forest → Risk Scores)
    ↓
SQLite Database (district_metrics table)
    ↓
FastAPI REST API
    ↓
React Frontend (MapLibre Map + Dashboards)
```
